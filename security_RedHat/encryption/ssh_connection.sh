...
# when connecting for first time, check fingerprint in remote machine, to see whether .pub key in the remote machine matches yours:
ssh-keygen -l -f .ssh/ssh_host_rsa_key.pub
...
ssh natasa@10.57.122.192 "cat >> ~/.ssh/known_hosts" < ~/.ssh/id_rsa.pub
# or:
ssh pepe@10.57.122.192 "mkdir ~/.ssh ; cat >> ~/.ssh/known_hosts" < ~/.ssh/id_rsa.pub
