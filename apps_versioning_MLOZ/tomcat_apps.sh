...
rm /data/${thisHostname}_listTomcatApps.txt
...
do
	...
	if [[ $nbwebapps -gt 0 ]]; then
		...
		echo "$applicationName - $versionNumber" >> /data/listTomcatApps.txt
	fi
done
scp /data/listTomcatApps.txt root@${destinationServer}:/data/scripts/
